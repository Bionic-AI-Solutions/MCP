...
- **Config file loading** via `from mcp_servers.common.config_loader import load_tenant_configs_from_file`
- Environment variable loading: `<UPPER_NAME>_TENANT_{id}_{CONFIG_KEY}` (legacy fallback for local dev)
- `_load_tenants_from_config_file()` reads `/etc/mcp/tenants.json`, returns `Dict[str, <Name>TenantConfig]`
- `initialize()` uses **3-tier loading priority**:
  ...
  - Later sources fill gaps, not overwrite — Redis tenants take precedence
...
Use the template at `template/src/mcp_servers/SERVER_NAME/tenant_manager.py` as the starting point — it already includes the config file loading pattern and 3-tier `initialize()` method.
...
- **ConfigMap** — non-secret settings: SERVER_NAME, REDIS_HOST, REDIS_PORT, REDIS_DB, PYTHONPATH
- **Deployment** — nodeSelector: ubuntu, imagePullSecrets, container with `envFrom` (ConfigMap only), `volumeMounts`, resources, probes
- **Service** — ClusterIP

**Volume mount for tenant secrets** (REQUIRED for multi-tenant servers):
```yaml
          volumeMounts:
            - name: tenant-config
              mountPath: /etc/mcp
      ...
      volumes:
        - name: tenant-config
          secret:
            secretName: mcp-<server-name>-tenants
            items:
              - key: tenants.json
                path: tenants.json
```

**DO NOT** put tenant credentials in the ConfigMap, env vars, or a Secret `stringData` block in the deployment YAML. All tenant configs go into a separate K8s Secret (`mcp-<server-name>-tenants`) managed outside of git.

Follow the exact structure of `k8s/redis/deployment.yaml` or `k8s/postgres/deployment.yaml`.

### Step 6: Create Tenant Secret

Create the K8s Secret that holds tenant configurations. This is the **only place** tenant credentials live.

**Create `k8s/<server-name>/tenants-secret.yaml`** (gitignored — never committed):

```yaml
apiVersion: v1
kind: Secret
metadata:
  name: mcp-<server-name>-tenants
  namespace: mcp
  labels:
    app: mcp-<server-name>-server
...
stringData:
  tenants.json: |
    {
      "base": {
        <config fields matching your TenantConfig model>
      }
    }
```

The JSON keys must match the Pydantic `TenantConfig` field names (minus `tenant_id`, which is injected from the key). Ask the user for tenant connection details.

**Apply to cluster:**
```bash
kubectl apply -f k8s/<server-name>/tenants-secret.yaml
```

**Verify the `.gitignore` includes the pattern** `k8s/*/tenants-secret.yaml` — it should already be there. If not, add it.

To add tenants later, use the `/manage-tenants` skill or update the secret directly:
```bash
kubectl get secret mcp-<server-name>-tenants -n mcp -o jsonpath='{.data.tenants\.json}' | base64 -d > /tmp/tenants.json
# ... edit /tmp/tenants.json ...
kubectl create secret generic mcp-<server-name>-tenants -n mcp \
  --from-file=tenants.json=/tmp/tenants.json --dry-run=client -o yaml | kubectl apply -f -
kubectl rollout restart deployment/mcp-<server-name>-server -n mcp
```

### Step 7: Create Kong Ingress Route
...
- Tenant secret name: `mcp-<server-name>-tenants`
- How to add tenants: update the K8s Secret JSON + `kubectl rollout restart`, or use `/manage-tenants create <tenant_id>`
- Pod status
- Verify with: `kubectl exec -n mcp deployment/mcp-<server-name>-server -- cat /etc/mcp/tenants.json`
...
### K8s Tenant Secret for Cluster Mode

Set `cluster_mode` in the tenant's JSON config inside the K8s Secret (`mcp-<server-name>-tenants`):
```json
{
  "my-tenant": {
...
### Redis Key Prefixing (Tenant Data Isolation)

Since Redis Cluster only supports DB 0, all tenants share the same keyspace. The Redis MCP server provides **automatic key prefixing** for tenant data isolation. Every key operation is transparently prefixed with `{tenant_id}:` so tenants cannot read or write each other's data.

**How it works:**
- `redis_set(tenant_id="vc", key="config", value="...")` → stores as `vc:config` in Redis
- `redis_get(tenant_id="vc", key="config")` → reads `vc:config` from Redis
- `redis_keys(tenant_id="vc", pattern="*")` → returns only keys matching `vc:*`, with the prefix stripped
- `redis_flushdb(tenant_id="vc", confirm=True)` → deletes only `vc:*` keys (not other tenants')
- `redis_dbsize(tenant_id="vc")` → counts only `vc:*` keys

**MCP clients never see the prefix** — it is added/removed transparently. Clients use plain key names (e.g. `config`, `user:123`) and the server handles namespacing.

**Config field:** The `key_prefix` field in `RedisTenantConfig` controls the prefix:
- Empty string (default) → auto-resolves to `{tenant_id}:`
- Custom value → used as-is (e.g. `"myapp:"`)
- `"__none__"` → disables prefixing entirely (keys are global)

**`redis_execute_command` bypasses prefixing** — it sends raw Redis commands and is the only tool that does not apply the tenant prefix.

### Stale Config Persistence Pitfall

Tenant configs are persisted in the internal Redis (DB assigned to the server). On startup, `initialize()` loads from Redis first, then the config file, then env vars. **If a stale config exists in Redis (e.g. with `cluster_mode: false`), it will be used instead of the updated config file value.**
...
Then restart the pod so it reloads from the config file.
...
All multi-tenant MCP servers use a **centralized JSON config file** for tenant credentials, mounted as a K8s Secret volume. This is the standard pattern — never put tenant credentials directly in deployment YAMLs, env vars, or ConfigMaps.

### How It Works

1. **Config file**: Each server reads `/etc/mcp/tenants.json` at startup
2. **K8s Secret**: The file comes from a Secret named `mcp-<server-name>-tenants` mounted as a volume
3. **Shared loader**: `src/mcp_servers/common/config_loader.py` provides `load_tenant_configs_from_file()`
4. **3-tier loading**: `initialize()` loads Redis → config file → env vars (later sources fill gaps only)
5. **Adding tenants**: Update the K8s Secret JSON + restart pod. No code or deployment YAML changes needed.

### Key Files

| File | Purpose |
|------|---------|
| `src/mcp_servers/common/config_loader.py` | Shared utility that reads `/etc/mcp/tenants.json` |
| `src/mcp_servers/common/__init__.py` | Package init for the common module |
| `template/src/mcp_servers/SERVER_NAME/tenant_manager.py` | Template with config file loading built in |
| `k8s/<server>/tenants-secret.yaml` | K8s Secret with tenant JSON (gitignored, never committed) |

### Anti-patterns to Avoid

- **Credentials in deployment YAML**: Never put passwords/keys in ConfigMap, env vars, or inline Secret `stringData`
- **Credentials committed to git**: All `tenants-secret.yaml` files are gitignored
- **Per-tenant env var blocks**: Don't add `*_TENANT_*_HOST`, `*_TENANT_*_PASSWORD` env vars — use the JSON config instead
- **Separate Secret per tenant**: Use one Secret per server with all tenants in a single JSON file

## Important Notes

- Always read existing server implementations for reference before writing code
- Use `template/src/mcp_servers/SERVER_NAME/tenant_manager.py` as the starting point for new tenant managers
...
- Use the `/manage-tenants` skill for adding/removing tenants after the server is deployed

## Installation

To install this skill in another Claude Code project:

```bash
mkdir -p .claude/skills/mcp-builder
cp mcp-builder.skill.md .claude/skills/mcp-builder/SKILL.md
```
